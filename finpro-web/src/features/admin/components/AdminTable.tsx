...
export type IAdminTableProps<T> = IAdminTablePropsWithCheckbox<T> | IAdminTablePropsWithoutCheckbox<T>;

export default function AdminTable<T extends Record<string, unknown>>(props: IAdminTableProps<T>) {
  const { title, tableDescription, linkHref, linkLabel, columns, data, renderCell, withCheckbox } = props;
  ...
  const checkedRows: string[] = isControlled
    ? (props as IAdminTablePropsWithCheckbox<T>).checkedRows!
    : internalChecked;

  const disabledRows = withCheckbox ? (props as IAdminTablePropsWithCheckbox<T>).disabledRows || [] : [];
              ...
              <th scope="col" className="p-4">
                <div className="flex items-center">
                  <input
                    ...
                    onChange={(event) => {
                      const allIds = event.target.checked ? data.map((row) => props.getRowId(row)) : [];
                  ...
                  <td className="w-4 p-4">
                    <div className="flex items-center">
                      <input
                        ...
                        className={`size-4 text-blue-600 bg-gray-100 border-gray-300 rounded-sm focus:ring-blue-500 focus:ring-2 ${
                          disabledRows.includes(rowId) ? 'opacity-50 cursor-not-allowed' : ''
                        }`}
                      />
